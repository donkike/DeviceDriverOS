...
static int __init init_driver(void) {
  ...
  if (cheater_major <= 0) {
    printk(KERN_WARNING "Problem registering device %s", cheater_name);
    return -EBUSY;
  }
  printk(KERN_INFO "Block device %s registered with major %d", cheater_name, cheater_major);
  ...
  printk(KERN_INFO "disk_name is %s", dev->gd->disk_name);
  set_capacity(dev->gd, nsectors * (hardsect_size / KERNEL_SECTOR_SIZE));
  add_disk(dev->gd);
  printk(KERN_INFO "disk_device %s added", dev->gd->disk_name);
  return SUCCESS;
 out_vfree:
  return -1;
}
